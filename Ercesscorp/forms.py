...
class LegalForm(forms.Form):
    ...
    def clean_gstin(self):
        ...
        if len(gst) != 15:
            raise forms.ValidationError('input your correct 15 digits GSTIN number')
    ...
    def clean_pan(self):
        ...
        if len(pan) != 10:
            raise forms.ValidationError('input your correct 10 digits PAN number')
        return pan

    def clean_cin(self):
        cin = self.cleaned_data.get('cin')
        if len(cin) != 21:
            raise forms.ValidationError('input your correct 21 digits CIN number')

    def clean(self):
        gst = self.cleaned_data('gstin')
        pan = self.cleaned_data('pan')
        if not pan in gst:
            raise forms.ValidationError('your PAN number is not in your GSTIN number')
